...
const nombresPadre = document.getElementById("nombre_padre");
const apellidoPadre = document.getElementById("apellido_padre");
const correoPadre = document.getElementById("correo_padre");
const celularPadre = document.getElementById("celular_padre");
...
const insertar = async (data) => {
  ...
  if (respuestaData.message === "rol invalido") {
    Swal.fire({
      icon: "error",
      title: "Rol invalido",
      text: "Por favor ingresa el rol para poder registrar al alumno",
    });
  } else if(respuestaData.message === 'dni duplicado') {
    Swal.fire({
      icon: "error",
      title: "DNI duplicado",
      text: "El DNI que estas intentando registrar ya existe en nuestra base de datos",
    });
  } else {
    Swal.fire({
      icon: "success",
      title: "Alumnos registrado",
      text: "El alumno a sido registrado con éxito",
    });
...
const modificar = async (data, id) => {
  ...
  if (respuestaData.message === "nombre vacio") {
    ...
    Swal.fire({
      ...
      text: "El alumno a sido modificado con éxito",
    });
    await obtnerRegistros();
    $("#exampleModal").modal("hide");
  }
...
const obtnerRegistros = async (search = "") => {
  ...
  if (respuestaData.data.length <= 0) {
    Swal.fire({
      icon: "warning",
      title: "Datos vacios",
      text: "Parece que no tienes alumnos registrados",
    });
  } else {
    ...
    .then((respuesta) => {
      if (respuesta.data.length <= 0) {
        Swal.fire({
          icon: "warning",
          title: "Datos vacios",
          text: "Parece que no tiene alumnos registrados",
        });
      } else {
        ...
        nombresPadre.value = registro.nombrePadre;
        apellidoPadre.value = registro.apellidoPadre;
        celularPadre.value = registro.celularPadre;
        correoPadre.value = registro.correoPadre;
      }
    });
};
...
const eliminar = async (id) => {
  Swal.fire({
    ...
    text: "Estas seguro que deseas eliminar al alumno!",
    icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      if (respuestaData.message === "error") {
        ...
        Swal.fire(
          "Usuario elminado!",
          "El alumno a sido eliminado de la base de datos",
          "success"
        );
        await obtnerRegistros();
      }
...
modal.addEventListener("submit", async (e) => {
  ...
  const data = {
    ...
    nombrePadre: nombresPadre.value,
    apellidoPadre: apellidoPadre.value,
    celularPadre: celularPadre.value,
    correoPadre: correoPadre.value,
  };
  if (action === "guardar") {
    await insertar(data);
  } else {
